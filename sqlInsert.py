...
# 股票代码6位数
def stockCode():
    return str(random.randint(1, 10000)).zfill(6)
...
# 随机百分比数字
def appendPercent(stock):
    return str(stock) + '%'


def stockRate(all=True):
    if all == True:
...
def stockDate():
    start_date = date.today().replace(day=1, month=6).toordinal()
    end_date = date.today().replace(day=30, month=6).toordinal()
    return str(date.fromordinal(random.randint(start_date, end_date)))
...
for _ in range(100):
    data = (stockCode(),
            stockName(),
            appendPercent(stockRate()),
            appendPercent(stockRate(all=False)),
            stockRate(all=False),
            stockRate(all=False),
            stockDate()
            )
    usersvalue.append(data)
...
sql = """insert into info (code, short, chg, turnover, price, highs, time) values (%s, %s, %s, %s, %s, %s, %s)"""

cursor.executemany(sql, usersvalue)
cursor.close()
db.commit()
db.close()
